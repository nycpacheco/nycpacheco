# Hello! I'm Nycolas Pacheco.

**Read in:** [Portuguese](README.md)

* **Systems Analysis and Development Student** — IFSP Bragança Paulista  
* **Software Development (Python & SQL)** — Focused on practical projects, automation, and API integration.  
* Seeking a **tech internship** opportunity.

##


### Technologies & Tools

* **Languages:** Python, SQL, C/C++, HTML5, CSS3, JavaScript
* **Frameworks & Libraries:** React (Vite), Flask, Bootstrap, Streamlit, Pandas, OpenPyXL
* **Data & AI:** SQLite, MySQL, Google Gemini API
* **Tools & Utilities:** Git, GitHub, VS Code

##


### Focus of Study

* **Backend & Data:** Building Python applications, relational modeling with SQL, and data manipulation.
* **Automation & APIs:** Integrating web services and language models to solve practical problems.
* **Interfaces & Prototypes:** Building interactive dashboards and web tools (Streamlit and React).

##


### Contact

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/nycolas-pacheco-b7934b3b5/)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:nycoajpacheco@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-4A5568?style=for-the-badge&logo=globe&logoColor=white)](https://nypacheco.github.io)

📧 **Email:** nycoajpacheco@gmail.com

##
